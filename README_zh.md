...
### 窗口缩放后面板尺寸超出 maxSize 限制

当某个面板设置了 `maxSize`，在较小的窗口中折叠了其他面板，然后扩大窗口时，该面板可能会超出其 `maxSize` 限制。此时尝试打开或关闭其他面板可能会报错。

**影响**：这不会影响正常使用。一旦其他面板占用了超出的空间，报错就会停止。

**解决方法**：手动调整该面板大小或与其他面板交互来重新分配空间。

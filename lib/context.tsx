    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      for (const [group, index] of ref.dragIndex.values()) {
        ...
        const tryExpandPanel = (
          panels: PanelValue[],
          remaining: number,
          maxShrintWithCollapse: number,
        ): boolean => {
          const nextPanel = panels.find((p) => p.collapsible && p.isCollapsed);
          if (
            nextPanel &&
            remaining > nextPanel.minSize / 2 &&
            nextPanel.minSize <= maxShrintWithCollapse
          ) {
        ...
        while (true) {
          const maxGrowBeforeNoExpand = panelsBefore.every((panel) => panel.isCollapsed)
            ...
            : panelsBefore.some((p) => !p.isCollapsed && p.maxSize === undefined)
              ...
              : panelsBefore.reduce(
                  (sum, p) =>
                    sum + (p.isCollapsed || p.maxSize === undefined ? 0 : p.maxSize - p.size),
                  0,
                );
          ...
          if (delta > 0) {
            let clampedGrow, clampedShrink;
            {
              ...
              if (
                remaining > 0 &&
                tryExpandPanel(panelsBefore, remaining, maxShrinkAfterWithCollapse)
              ) {
            ...
            }
            {
              ...
              if (
                remaining > 0 &&
                tryCollapsePanel(panelsAfter, remaining, maxGrowBeforeWithExpand)
              ) {
          ...
          if (delta < 0) {
            let clampedGrow, clampedShrink;
            {
              ...
              if (
                remaining > 0 &&
                tryExpandPanel(panelsAfter, remaining, maxShrinkBeforeWithCollapse)
              ) {
                // Continue loop to recalculate clamped with expanded panel
                continue;
              }
            ...
            }
            {
              ...
              if (
                remaining > 0 &&
                tryCollapsePanel(panelsBefore, remaining, maxGrowAfterWithExpand)
              ) {
                // Continue loop to recalculate clamped with new space
                continue;
              }
